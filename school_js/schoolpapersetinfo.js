...
$(document).ready(function () {
    ...
    $(document).on('change', '#txtcode', function () {
        if ($('#txtcode').val() != "") {
            ...
            var path = "SchoolPaperSetDetail.aspx/CodeExist";
            var status = "";
...
function CheckCodeAvailable(code, path) {
    ...
    $.ajax({
        ...
        url: path,
        data: JSON.stringify({ Code: code, ClassID: classid,SectionID:sectionid,SubjectID:subjectid }),
        dataType: "json",
        async: false,
...
function DeleteDetailListByID(id) {
    $.ajax({
        type: "POST",
        contentType: "application/json; charset=utf-8",
        url: "SchoolPaperSetDetail.aspx/DeleteDetailListByID",
...
function GetDetailListByID(id) {
    $.ajax({
        type: "POST",
        contentType: "application/json; charset=utf-8",
        url: "SchoolPaperSetDetail.aspx/GetSectionDetailByID",
        data: JSON.stringify({ ID: id }),
        dataType: "json",
        async: false,
        success: function (response) {
...
function Save() {
    if ($("#txtcode").val() == "") {
        ...
        alertify.log("PaperSet is required.");
        $("#txtpaperset").focus();
        return false;
    }

    var obj = {};
    obj.PaperSetID = $('#hdfpapersetid').val();
    obj.ClassID = $('#ddlclass').val();
    obj.ClassSectionID = $('#ddlsection').val();
    obj.SubjectID = $('#ddlsubject').val();
    obj.PaperSetCode = $('#txtcode').val();
    obj.PaperSetName = $('#txtpaperset').val();
    obj.Active = $('#ddlstatus').val();
    $.ajax({
        type: 'POST',
        contentType: "application/json; charset=utf-8",
        url: 'SchoolPaperSetDetail.aspx/Save',
...
function BindGrid() {
    $.ajax({
        type: "POST",
        contentType: "application/json; charset=utf-8",
        url: "SchoolPaperSetDetail.aspx/GetPaperSetDetail",
        data: JSON.stringify({ }),
        dataType: "json",
        contentType: "application/json; charset=utf-8",
        success: FillJsonXML,
    ...
    function FillJsonXML(data) {
        $("#gv").empty();
        if (data.d.length > 0) {
            $("#gv").append("<thead><tr><th style='width:15px' class='center'>Action</th><th>Class</th><th>Section</th><th>Subject</th><th>Code</th><th>PaperSet Name</th><th>Status</th>" +
                "</tr></thead><tbody>");
            for (var i = 0; i < data.d.length; i++) {
                ...
                $("#gv").append("<tr><td class='" + activecolor + "'>" +
                        ...
                        "  <a class='list_toolbr_btn lnkedit' data-id='" + data.d[i].PaperSetID + "' href='javascript:void();'> Edit</a>" +
                        " </div> </li><li> <div class='list_toolbr'> <i class='fa fa-info-circle 2x' aria-hidden='true'></i>" +
                        "  <a class='list_toolbr_btn lnkdelete' data-id='" + data.d[i].PaperSetID + "' href='javascript:void();'> Delete</a>" +
                        " </div> </li></ul></div>" +
                       "</td>" +
                        ...
                        "<td class='" + activecolor + "'>" + data.d[i].PaperSetCode + "</td><td class='" + activecolor + "'>" + data.d[i].PaperSetName + "</td><td class='" + activecolor + "'>" + active + "</td></tr>");
            }
            $('#gv').append("</tbody>");
            Initialize();
        }
        else {
            $("#gv").append("<thead><tr><th>Paper Set Detail List</th></tr></thead>");
...
function BindCombo(ele, ControlName) {
    ...
    $.ajax({
        type: "POST",
        contentType: "application/json; charset=utf-8",
        url: "SchoolPaperSetDetail.aspx/BindComboBox",
