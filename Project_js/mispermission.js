...
function SaveSetting() {
    ...
    $.ajax({
        ...
        url: "MISPermission.aspx/SavePermission",
...
function UpdatePermission(SID, Tag, TagValue) {
    ...
    $.ajax({
        type: "POST",
        contentType: "application/json; charset=utf-8",
        url: "MISPermission.aspx/UpdatePermissionDetail",
...
function BindGrid() {
    ...
    $.ajax({
        type: "POST",
        contentType: "application/json; charset=utf-8",
        url: "MISPermission.aspx/GetPermissionDetail",
        data: JSON.stringify({ RoleID: RoleID, UserID: UserID }),
        dataType: "json",
        contentType: "application/json; charset=utf-8",
        success: FillJsonXML,
    ...
    function FillJsonXML(data) {
        $("#gv").empty();
        if (data.d.length > 0) {
            $("#gv").append("<thead><tr><th style='display:none'>SNo.</th><th>MIS</th><th>ReportName</th><th style='display:none'>Status</th><th>MISProcedure </th><th>On/Off </th>" +
                "</tr></thead><tbody>");
            for (var i = 0; i < data.d.length; i++) {
                ...
                $("#gv").append("<tr><td style='display:none'>" + data.d[i].SettingPermissionID + "</td>" +
                   ...
                   "<td  class='" + coloraccess + "' style='width:20%'>" + data.d[i].DefaultValue + "</td>" +
