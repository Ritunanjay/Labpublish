...
function SaveCentrePermissionDetail(id) {
    $.ajax({
        ...
        url: "DoctorGroupPermission.aspx/SaveCentrePermissionList",
...
function UpdateDetail(id, status) {
    $.ajax({
        type: "POST",
        contentType: "application/json; charset=utf-8",
        url: "DoctorGroupPermission.aspx/UpdateDetail",
...
function FillUserDetail() {
    $.ajax({
        type: "POST",
        contentType: "application/json; charset=utf-8",
        url: "DoctorGroupPermission.aspx/FillUserDetail",
        data: JSON.stringify({}),
        dataType: "json",
        contentType: "application/json; charset=utf-8",
        success: FillJsonXML,
    ...
    function FillJsonXML(data) {
        ...
        $("#gv").empty();
        if (data.d.length > 0) {
            $("#gv").append("<thead><tr><th style='background:linear-gradient(45deg, black, transparent);color:white;'>Doctor Detail</th></tr></thead>");
...
function FillCentreDetail(id) {
    $.ajax({
        type: "POST",
        contentType: "application/json; charset=utf-8",
        url: "DoctorGroupPermission.aspx/FillCentreDetail",
        data: JSON.stringify({ ID: id }),
        dataType: "json",
        contentType: "application/json; charset=utf-8",
        success: FillJsonXML,
    ...
    function FillJsonXML(data) {
        ...
        $("#gvcentre").empty();
        if (data.d.length > 0) {
            $("#gvcentre").append("<thead><tr><th style='background:linear-gradient(45deg, black, transparent);color:white;'>Action</th><th>Doctor Name</th><th>Permission</th><th>Group Detail</th></tr></thead>");
            for (var i = 0; i < data.d.length; i++) {
                ...
                $("#gvcentre").append("<tr class='" + cssstatus + "' ><td><input  class='form-control lnkcentreselect' style='cursor:pointer;'  data-id='" + data.d[i].CentreID + "' type='checkbox' " + status + " /></td><td> " + username + "</td><td> " + permission + "</td><td>" + data.d[i].Code + " | " + data.d[i].CentreName + "</td></tr>");
            }
            Initialize();
        }
        else {
            $("#gvcentre").append("<thead><tr><th>Group Detail List</th></tr></thead>");
