...
$(document).ready(function () {
    ...
    $(document).on('click', '.lnkbookingedit', function () {
        var billid = $(this).data('billid');
        checkslipdetail(billid);
        GetBookingDetail(billid);
        $('#ModalPatientBookingDetail').modal('hide');
    });
    PermissionDetail();
    ...
    $("#txtsearchpatient").autocomplete({
        source: function (request, response) {
            $.ajax({
                ...
                url: "PatientBookingModify.aspx/SearchPatient",
    ...
    $("#txtitemsearch").autocomplete({
        source: function (request, response) {
            $.ajax({
                type: "POST",
                contentType: "application/json; charset=utf-8",
                url: "PatientBookingModify.aspx/SearchInvestigation",
    ...
    $(document).on('click', '#btnadd', function () {
        if (confirm("Do You Want Save Booking! With Net Amount Rs. " + $('#txtnet').val()))
               SaveBooking();
    });
...
function checkslipdetail(billid)
{
    $.ajax({
        type: 'POST',
        contentType: "application/json; charset=utf-8",
        url: 'PatientBookingModify.aspx/Getslipdetail',
        data: JSON.stringify({ BillID: billid }),
        success: function (response) {
            if (response.d != "Error Found") {
                if (response.d == "0") {
                    alertify.error('Booking Has More Than One Slip Please Cancel');
                    return;
                }
                else {
...
function DeleteBooking(id) {
    $.ajax({
        type: "POST",
        contentType: "application/json; charset=utf-8",
        url: "PatientBookingModify.aspx/DeleteBooking",
...
function DiscountInvestigation() {
    $.ajax({
        type: "POST",
        contentType: "application/json; charset=utf-8",
        url: "PatientBookingModify.aspx/DiscountInvestigation",
...
function PrintBooking(id) {
    $.ajax({
        type: 'POST',
        contentType: "application/json; charset=utf-8",
        url: 'PatientBookingModify.aspx/PrintPathologyBill',
        data: JSON.stringify({ ID: id }),
        async: false,
        success: function (response) {
...
function CheckCouponCode(code) {
    $.ajax({
        type: "POST",
        contentType: "application/json; charset=utf-8",
        url: "PatientBookingModify.aspx/CheckCouponCode",
...
function GetDigitalCardBalance() {
    $.ajax({
        type: "POST",
        contentType: "application/json; charset=utf-8",
        url: "PatientBookingModify.aspx/GetDigitalCardBalance",
...
function getgenderdetail() {
    $.ajax({
        type: "POST",
        contentType: "application/json; charset=utf-8",
        url: "PatientBookingModify.aspx/GetGenderDetail",
...
function SaveInvestigation(itemid, qty) {
    ...
    $.ajax({
        type: "POST",
        contentType: "application/json; charset=utf-8",
        url: "PatientBookingModify.aspx/SaveInvestigation",
...
function GetPatientByID(id) {
    $.ajax({
        type: "POST",
        contentType: "application/json; charset=utf-8",
        url: "PatientBookingModify.aspx/GetPatientByID",
        data: JSON.stringify({ ID: id }),
        dataType: "json",
        async: false,
        success: function (response) {
...
function BindCombo(ele, ControlName, code) {
    ...
    $.ajax({
        type: "POST",
        contentType: "application/json; charset=utf-8",
        url: "PatientBookingModify.aspx/BindComboBox",
...
function DeleteRow(RowIndex) {
    debugger; $.ajax({
        type: "POST",
        contentType: "application/json; charset=utf-8",
        url: "PatientBookingModify.aspx/DeleteRow",
...
function SaveBooking() {
    if (parseInt($('#hdfbillid').val()) == 0)
    {
        alertify.error("Please Select Booking First !!!");
        return false;
    }
    ...
    if (Validate()) {
        ...
        robj.Remark = $('#txtupdateremark').val();
        ...
        $.ajax({
            type: 'POST',
            contentType: "application/json; charset=utf-8",
            url: 'PatientBookingModify.aspx/CheckCreditAmount',
...
function SavebookingDetail(pobj, objr, bobj, robj) {
    $.ajax({
        type: 'POST',
        contentType: "application/json; charset=utf-8",
        url: 'PatientBookingModify.aspx/SaveBooking',
...
function FillPatientBindGrid() {
    $.ajax({
        type: "POST",
        contentType: "application/json; charset=utf-8",
        url: "PatientBookingModify.aspx/GetBookingPatientDetail",
    ...
    function FillJsonXML(data) {
        $("#gvbooking").empty();
        if (data.d.length > 0) {
            $("#gvbooking").append("<thead><tr><th style='width:15px' class='center'>Action</th><th style='width:15px' class='center'>Cancel_Booking</th><th style='width:15px' class='center'>Print</th><th style='width:15px' class='center'>BillNo</th><th>UHID</th>" +
            ...
            for (var i = 0; i < data.d.length; i++) {
                ...
                billmodify = "<a class='fa fa-edit lnkbookingedit' style='font-size: large;' data-uhid='" + data.d[i].UHID + "' data-patientname='" + data.d[i].PatientName + "' data-billno='" + data.d[i].BillNo + "' data-billid='" + data.d[i].BillID + "' href='javascript:void();'></a>";
               if (data.d[i].Status == 'C') {
                    ...
                    if (print == 'False')
                        billdetail = '';
                    color = successcolor;
                    $("#gvbooking").append("<tr style='background-color:" + color + "'><td>" + billmodify + "</td>" +
                        "<td>" + billdelete + "</td>" +
                        "<td>" + billdetail + "</td>" +
                    ...
                    if (print == 'False')
                        billdetail = '';
                    color = failcolor;
                    $("#gvbooking").append("<tr style='background-color:" + color + "'><td>" + billmodify + "</td><td>" + billdelete + "</td>" +
                        "<td>" + billdetail + "</td>" +
                        "<td>" + data.d[i].BillNo + "</td>" +
                             "<td>" + data.d[i].UHID + "</td>" +
                             "<td>" + data.d[i].PatientName + "</td>" +
...
function ClearBookingItem() {
    $.ajax({
        type: 'POST',
        contentType: "application/json; charset=utf-8",
        url: 'PatientBookingModify.aspx/EmptyBookingTestList',
        data: JSON.stringify({}),
        async: false,
        success: function (response) {
...
function savedoctor() {
    ...
    $.ajax({
        type: 'POST',
        contentType: "application/json; charset=utf-8",
        url: 'PatientBookingModify.aspx/SaveDoctor',
...
function GetCentreRegistrationNo(id) {
    $.ajax({
        type: 'POST',
        contentType: "application/json; charset=utf-8",
        url: 'PatientBookingModify.aspx/GetCentreRegistrationNo',
        data: JSON.stringify({ ID: id }),
        async: false,
        success: function (response) {
...
function GetBookingDetail(billid) {
    $.ajax({
        type: 'POST',
        contentType: "application/json; charset=utf-8",
        url: 'PatientBookingModify.aspx/GetBookingDetail',
        data: JSON.stringify({ BillID: billid }),
        async: false,
        success: function (response) {
            setpatientdetail(response.d.objPatient);
            setinvestigationlist(response.d.objbookingOrder);
            setbilldetail(response.d.objbooking);
            setreceiptdetail(response.d.objReceipt);
            $('#btnadd').css('display', '');
            $('#btnadd').val('Update Booking');
        },
        error: function (jqXHR, status, errorThrown) {
            if (jqXHR.status == "403") {
                window.location.href = jqXHR.responseText.substring(0, jqXHR.responseText.indexOf('{'));
...
function setpatientdetail(data) {
    $("#txtsearchpatient").val('');
    $('#lnkname').html('Patient Name: [' + data.PatientName + ']');
    $('#lnkuhid').html('Patient UHID: [ ' + data.UHID + ']');
    $('#lnkpatientdetail').html('Other Detail:  [ ' + data.AgeYear + 'Y/ ' + data.AgeMonth + 'M/ ' + data.AgeDay + 'D/ ' + ' Gender :' + data.GenderID + ']');
    $('#hdfpatientid').val(data.PatientID.toString());
    $('#txtsearchpatient').val('');
    $('#hdfcentreid').val(data.CentreID);
    $('#hdfdoctorid').val(data.DoctorID);
    $('#hdfcategoryid').val(data.PatientCategoryID);
    $('#hdfrecordid').val(data.RecordID);
    $('#hdftpasponserid').val(data.TPASponserID);
}
function setinvestigationlist(data) {
    if (data == null) {
        alertify.error('Test Already Exist');
        return;
    }
    ...
    if (data.length > 0) {
        ...
        for (var i = 0; i < data.length; i++) {
            $("#gvlaborder").append("<tr><td>" +
                "<a class='fa fa-times-circle-o lnkdelete' data-id='" + data[i].ItemID + "' href='javascript:void();' style='font-size: x-large;'></a>" +
               "</td>" +
                "<td class='center'>" + data[i].ProfileInfo + "</td><td class='center'>" + data[i].TestCode + "</td><td class='center'>" + data[i].TestName + "</td><td><i class='lnkrate'>" + data[i].ItemRate + "</i></td><td><i class='lnkqty'>" + data[i].ItemQty + "</i></td>" +
                "<td><i class='lnkamount'>" + data[i].ItemNetAmount + "</i></td></tr>");
        }
        $('#gvlaborder').append("</tbody>");
        CalculateTotal();
    }
...
function setbilldetail(data) {
    $('#ddldiscountby').val(data.DiscountBy);
    $('#ddlextraby').val(data.ExtraBy);
    if ($('#ddlextraby').val() == "0") {
        $('#txtextra').attr('disabled', 'disabled');
        $('#txtextra').val('0');
        calculation();
    ...
    } $('#ddlcentre').val(data.CentreID);
    $('#hdfbillid').val(data.BillID);
    $('#hdfpatientid').val(data.RefID);
    $('#ddlcreditor').val(data.CreditorID);
    $('#txtregdate').val(data.BillOpenDate);
    $('#txttotal').val(data.BillAmount);
    $('#txtdiscount').val(data.DiscountAmount);
    $('#txtextra').val(data.TaxAmount);
    $('#txtnet').val(data.NetAmount);
    calculatepercentage();
...
function setreceiptdetail(data) {
    $('#hdfreceiptid').val(data.ReceiptID);
    $('#txtnet').val(data.PayableAmount);
    $('#txtpaid').val(data.PaidAmount);
    $('#ddlpaymode').val(data.Paymode);
    $('#txtregdate').val(data.ReceiptDate);

    $('#chckcash').prop('checked', data.IsCash);
    $('#chckcard').prop('checked', data.IsCard);
    $('#chckcheque').prop('checked', data.IsCheque);
    $('#chckewallet').prop('checked', data.IsEwallet);
    $('#txtcashamount').val(data.CashAmount);
    $('#txtcardamount').val(data.CardAmount);
    $('#txtcardno').val(data.CardNumber);
    $('#txtcardholdername').val(data.CardHolderName);
    $('#ddlcardbank').val(data.CardBankID);
    $('#txtcardtransactionno').val(data.CardTransactionNo);
    $('#txtcarddate').val(data.CardDate);
    $('#ddlcardmachine').val(data.MachineID);
    $('#txtchequeamount').val(data.ChequeAmount);
    $('#txtchequeno').val(data.ChequeNumber);
    $('#txtchequedate').val(data.ChequeDate);
    $('#ddlchequebank').val(data.ChequeBankID);
    $('#txtewalletamount').val(data.EwalletAmount);
    $('#ddlewallettype').val(data.EwalletTypeID);
    $('#txtewallettransactionno').val(data.EwalletTransactionNo);
    $('#txtewalletdate').val(data.EwalletDate);
