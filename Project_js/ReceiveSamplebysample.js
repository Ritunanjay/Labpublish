...
function RejectSample(received) {
    ...
    $.ajax({
        ...
        url: "ReceiveSample.aspx/SaveSampleCollect",
...
function GetSampleDetail(id, billid, sampleid) {
    $.ajax({
        type: "POST",
        contentType: "application/json; charset=utf-8",
        url: "ReceiveSample.aspx/GetSampleDetail",
...
function BindCombo(ele, ControlName) {
    ...
    $.ajax({
        type: "POST",
        contentType: "application/json; charset=utf-8",
        url: "ReceiveSample.aspx/BindComboBox",
...
function BindGrid() {
    $.ajax({
        type: "POST",
        contentType: "application/json; charset=utf-8",
        url: "ReceiveSample.aspx/GetSampleReceivedDetail",
    ...
    function FillJsonXML(data) {
        ...
        if (data.d.length > 0) {
            $('#gvsamplecollection').append("<thead><tr><th style='width:15px' class='center'>Barcode</th><th style='width:15px' class='center'>Select</th><th style='width:15px' class='center'>Reject</th><th>CentreName</th>"+
                "<th>UHID</th><th>PatientName</th><th>BillNo</th><th>TestList</th><th>Sample</th><th>SampleNo</th><th>ReceivedDate</th>"+
                "<th>CollectionDate</th><th>RejectedDetail</th><th>Container</th><th>ContainerNo</th><th>ContainerRemark</th><th>IsLabTransfer</th><th>IsSampleCollect</th></tr></thead><tbody>");
            for (var i = 0; i < data.d.length; i++) {
                ...
                var labtransfer = (data.d[i].IsLabTransfer != true) ? "Lab Transfer" : "Not Lab Transfer";
                var samplecollect = (data.d[i].IsSampleCollect != true) ? "Sample Collect" : "Sample Not Collect";
                $("#gvsamplecollection").append("<tr style='color:" + samplereceive + "'><td>" + samplebarcode + "</td><td>" +
...
function BindSampleDetail(billid,sampleid) {
    $.ajax({
        type: "POST",
        contentType: "application/json; charset=utf-8",
        url: "ReceiveSample.aspx/BindSampleDetail",
        ...
        if (data.d.length > 0) {
            $('#gvsampledetail').append("<thead><tr>"+
              //  "<th style='width:15px' class='center'>Select</th><th style='width:15px' class='center'>Reject</th>"+
                "<th>UHID</th><th>PatientName</th><th>OrderNo</th><th>TestName</th><th>Sample</th><th>SampleNo</th><th>ReceivedDate</th><th>CollectionDate</th><th>RejectedDetail</th></tr></thead><tbody>");
            for (var i = 0; i < data.d.length; i++) {
                var samplereceive = (data.d[i].Received == 0) ? 'red' : 'green';
                var rejectdetail = (data.d[i].Received != false) ? "<a class='fa fa-times-circle-o lnkreject' style='font-size: large;' data-billid='" + data.d[i].BillID + "'  data-uhid='" + data.d[i].UHID + "' data-patientname='" + data.d[i].PatientName + "' data-ordernumber='" + data.d[i].OrderNo + "'  data-sampleid='" + data.d[i].SampleID + "' data-id='" + data.d[i].SCID + "' data-orderid='" + data.d[i].OrderID + "' href='javascript:void();'></a>" : "";
