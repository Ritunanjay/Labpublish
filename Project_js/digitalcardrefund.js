$(document).ready(function () {
    ...
    $("#txtsearchpatient").autocomplete({
        source: function (request, response) {
            $.ajax({
                ...
                url: "DigitalCardRefund.aspx/SearchPatient",
    ...
    $(document).on('click', '#btnadd', function () {
        ...
        if (parseFloat(balance) < paid) {
            alertify.error('Please Fill Less Than Balance Amount');
            return;
        } else
            SaveDigital();
...
function calculation() {
    ...
    var paid = $('#txtpaid').val();
    if (parseFloat(balance) < paid) {
        if (confirm('Please Fill Less Than Balance Amount'))
            return false;
        else
            return true;
    }
...
function GetPatientByID(id) {
    $.ajax({
        type: "POST",
        contentType: "application/json; charset=utf-8",
        url: "DigitalCardRefund.aspx/GetPatientByID",
...
function setValues(response) {
    ...
    patientbalance(response.d.RecordID)
}
function patientbalance(id) {
    $.ajax({
        type: "POST",
        contentType: "application/json; charset=utf-8",
        url: "DigitalCardRefund.aspx/GetPatientBalance",
        data: JSON.stringify({ ID: id }),
        dataType: "json",
        async: false,
        success: function (response) {
...
function PrintBill(id) {
    $.ajax({
        type: 'POST',
        contentType: "application/json; charset=utf-8",
        url: 'DigitalCardRefund.aspx/PrintDigitalDetail',
        data: JSON.stringify({ ID: id }),
        async: false,
        success: function (response) {
...
function BindGrid() {
    $.ajax({
        type: "POST",
        contentType: "application/json; charset=utf-8",
        url: "DigitalCardRefund.aspx/GetDigitalCardDetail",
...
function SaveDigital() {
    ...
    dobj.PaidType = 'Debit';
    dobj.AdvanceNo = '';
    dobj.Narration = 'Digital Card Refund';
    ...
    $.ajax({
        type: 'POST',
        contentType: "application/json; charset=utf-8",
        url: 'DigitalCardRefund.aspx/SaveDigitalCard',
