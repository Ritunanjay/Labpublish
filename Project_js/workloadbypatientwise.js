$(document).ready(function () {
    ...
    $(document).on('click', '.lnkselect', function () {
        ...
        ShowStructure(billid);

    });
    $(document).on('click', '.lnkprint', function () {
        var BillID = $(this).data('billid');
        var status = $(this).data('status');
        PrintReport(BillID,status);
    });
    ...
    $(document).on('click', '#chckvalidate', function () {
        ...
        ReportValidate(billid, status);
    });
});
function ReportValidate(billid, status) {
    $.ajax({
        type: 'POST',
        contentType: "application/json; charset=utf-8",
        url: 'WorkloadByPatientWise.aspx/ReportValidate',
...
function PrintReport(id,status) {
    $.ajax({
        type: 'POST',
        contentType: "application/json; charset=utf-8",
        url: 'WorkloadByPatientWise.aspx/PrintPathologyBillByReport',
...
function ShowStructure(billid) {
    $.ajax({
        type: "POST",
        contentType: "application/json; charset=utf-8",
        url: "WorkloadByPatientWise.aspx/ShowDetail",
...
function BindGrid() {
    $.ajax({
        type: "POST",
        contentType: "application/json; charset=utf-8",
        url: "WorkloadByPatientWise.aspx/GetWorkloadDetail",
    ...
    function FillJsonXML(data) {
        $("#gvpathology").empty();
        if (data.d.length > 0) {
            $('#gvpathology').append("<thead><tr><th style='width:15px' class='center'>Validate</th><th style='width:15px' class='center'>Select</th><th style='width:15px' class='center'>Print</th><th>Centre_Detail</th><th>UHID</th><th>PatientName</th><th>Mobile</th><th>EmailID</th><th>BillNo</th><th>TestList</th><th>Booking_Date</th><th>DoctorName</th><th>TATTimer</th></tr></thead><tbody>");
            for (var i = 0; i < data.d.length; i++) {
                ...
                if (data.d[i].Recheck == 'B')
                    ...
                    printdetail = " <a class='glyphicon glyphicon-print lnkprint'  data-status='I' data-billid='" + data.d[i].BillID + "' style='font-size: large;' href='javascript:void();'> </a>";
                } else if (data.d[i].Recheck == 'C') {
                    fillreporting = '#39ec3f6b';
                    printdetail = " <a class='glyphicon glyphicon-print lnkprint'  data-status='C' data-billid='" + data.d[i].BillID + "' style='font-size: large;' href='javascript:void();'> </a>";
                    timer = 'Test Complete Done';
                } else if (data.d[i].Recheck == 'V') {
                    fillreporting = '#c839ec6b';
                    printdetail = " <a class='glyphicon glyphicon-print lnkprint'  data-status='V' data-billid='" + data.d[i].BillID + "' style='font-size: large;' href='javascript:void();'> </a>";
                    reportvalidate = "checked";
                    timer = 'Test Validate Done';
                } else if (data.d[i].Recheck == 'P') {
                    fillreporting = '#b3851d6b';
                    printdetail = " <a class='glyphicon glyphicon-print lnkprint'   data-status='P' data-billid='" + data.d[i].BillID + "' style='font-size: large;' href='javascript:void();'> </a>";
                    reportvalidate = "checked";
                    timer = 'Test Validate Done';
                }
